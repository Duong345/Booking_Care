        ...
        users = users.map((u) => {
          if (u.image) {
            if (typeof u.image !== "string") {
              // sometimes Sequelize returns Buffer-like object
              if (u.image.data) {
                u.image = Buffer.from(u.image.data).toString("base64");
              } else {
                u.image = u.image.toString();
              }
            }
            if (!u.image.startsWith("data")) {
              u.image = `data:image/jpeg;base64,${u.image}`;
            }
          }
          return u;
        });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.arrSchedule || !data.doctorId || !data.formatedDate) {
        ...
        // Create new slots that don't exist yet
        ...
        // Delete slots that were deselected (present in existing but not in schedule)
        let toDelete = existing.filter((e) => {
          return !schedule.find(
            (s) => s.timeType === e.timeType && +s.date === +e.date
          );
        });
        ...
        for (let del of toDelete) {
          // make sure there is no active booking for this slot
          let bookings = await db.Booking.findAll({
            where: {
              doctorId: data.doctorId,
              date: data.formatedDate,
              timeType: del.timeType,
              statusId: { [Op.ne]: "S4" }, // not canceled
            },
            raw: true,
          });
          if (bookings && bookings.length > 0) {
            // skip deletion if there are bookings
            skipped.push(del.timeType);
            continue;
          }
        ...
        let message = `OK. Created: ${toCreate.length}, Deleted: ${deletedCount}`;
        if (skipped.length > 0) {
          message += `; Skipped deletion for timeTypes: ${skipped.join(", ")}`;
        }
    ...
    try {
      if (!doctorId || !date) {
        ...
        let data = await db.Booking.findAll({
          ...
          attributes: ["id", "statusId", "doctorId", "patientId", "date", "timeType", "reason"],
          include: [
            {
              model: db.User,
              as: "patientData",
...
let getConfirmedBookingHistory = (doctorId) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId) {
        resolve({
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.email || !data.doctorId || !data.patientId || !data.timeType || !data.date) {
        ...
        console.log('Finding appointment:', data.doctorId, data.patientId, data.timeType, data.date, 'S2');
        console.log('Appointment found:', appointment);
        let doctorName = "";
        let timeText = data.timeType;
        if (appointment) {
        ...
        if (doctor) doctorName = `${doctor.firstName} ${doctor.lastName}`;
        let emailData = {
          email: data.email,
        ...
        // send cancel email (no attachment)
        await emailService.sendCancelEmail(emailData);
        resolve({
          errCode: 0,
          errMessage: "OK",
        });
...
module.exports = {
  ...
  getConfirmedBookingHistory: getConfirmedBookingHistory,
};
